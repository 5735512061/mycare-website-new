      ...
      <div class="right" style="margin-top: -580px !important; height:520px !important;">
        <form class="sendurl">
            <div class="input-group form-group">
                <input type="text" id="serialnumber1" maxlength="19" minlength="19" name="serialnumber" placeholder="กรอกหมายเลขบัตรสมาชิก 16 หลัก" onfocus="this.placeholder = 'กรอกหมายเลขบัตรสมาชิก 16 หลัก'" class="single-input{{ $errors->has('serialnumber') ? ' is-invalid' : '' }}">
...
$('.sendurl').submit(function(e){
    ...
    $.ajax({
        type: "POST",
        url: "{!! url('/privilege/expire/store/receive') !!}",
        data: {
            ...
            success: function(response) {
                ...
                // }
                    ...
                    $('#tag-id').html('<p style="text-align: center; font-weight: bold; color:#0517c4;">สิทธิพิเศษสำหรับ คุณ'+response.member.name+'</p><p style="text-align: center; color:#0517c4;">คุณได้'+response.store.promotion+'</p><p style="text-align: center; color:#0517c4;">และรับคะแนนสะสม '+response.store.point+' คะแนน</p><p style="text-align: center; background-color:#0517c4; color:#ffffff; margin:0px 80px 0px 80px;">'+response.statistic.code+'</p><p style="margin-top:5px;font-size:15px; text-align: center; color:#979797;">* กรุณาแสดงรหัส คู่บัตร MY CARE ต่อพนักงาน เพื่อยืนยันการสิทธิ์</p>')
                }
