...
<div class="container"> 
    <div class="row">
        ...
        <div class="col-lg-4">
            <a href="{{url('/member/service-history/')}}/{{$value->id}}">
                <div class="offer offer-radius offer-primary">
                    <div class="shape"></div>
                    <div class="offer-content">
                        <p>ทะเบียน : {{$value->carnumber}} {{$value->carprovince}}</p>
                        <p class="mbr-fonts-style text1 mbr-text display-6">
                            {{$value->brand}} {{$value->model}} สี{{$value->color}}
                        </p>
                        <p class="mbr-fonts-style text1 mbr-text display-6">
                            <?php 
                                $miles = DB::table('services')->where('member_id',$value->id)
                                        ->orderBy('date','desc')->value('miles');
                            ?>
                            เลขไมล์ล่าสุด : {{$miles}}
                        </p>
                        <?php $mile = $miles+10000; ?>
                        <p style="color: red;">ถ่ายน้ำมันเครื่องที่ {{$mile}} กม</p>
                    </div>
                </div>
            </a>
        </div>
