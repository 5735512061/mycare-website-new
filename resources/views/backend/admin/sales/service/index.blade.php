    ...
    <div class="row">
        <div class="col-md-12">
            <div class="panel-heading">
                <P><a class="genric-btn info radius" href="{{url('/admin/createService-sales')}}/{{$member_id}}">เพิ่มข้อมูลบริการ</a></P>
            </div>
            <table class="table table-bordered table-striped">
                ...
                <tbody class="table">
                    <tr>
                        ...
                        <td><a style="color: #000;" href="{{url('/admin/serviceInformation-sales/')}}/{{$value->id}}">{{$value->date}}</a></td>
                        <?php 
                            $list = DB::table('sales_services')->where('date',$value->date)
                                                               ->where('car_id',$value->car_id)
                                                               ->count();
                        ?>
                        <td><a style="color: #000;" href="{{url('/admin/serviceInformation-sales/')}}/{{$value->id}}">{{$list}} รายการ <i class="fa fa-pencil-square" style="color:blue;"></a></td>
                        <?php 
                            $sum = 0;
                        ...
                        <td style="color: #000;">
                            <center>
                                <a href="{{url('/admin/serviceInformation-sales/')}}/{{$value->id}}">
