	...
	<div class="container">
		<div class="row d-flex align-items-center justify-content-between" style="height:250px !important;">
			<div class="home-banner-content col-lg-12 col-md-12">
				<h2 style="text-align:center; color:#092895;">สมัครสมาชิกผ่านเว็บไซต์</h2><hr style="border: 1px solid blue;">
			</div>
		</div>
	</div>
</section>
                    ...
                    @foreach($members as $member => $value)
                        <tbody>
                            <tr>
                                ...
                                <?php 
                                    $comment = DB::table('payments')->where('member_id',$value->id)->value('comment');  
                                ?>
                                @if($value->serialnumber == NULL && $comment != "ข้อมูลไม่ถูกต้อง")
                                <td>
                                    <a href="{{url('/admin/member-information/')}}/{{$value->id}}" class="btn btn-warning"><i class="fa fa-exclamation-triangle"></i> ตรวจสอบข้อมูล</a>
                                </td>
                                @elseif($comment == "ข้อมูลไม่ถูกต้อง" && $value->serialnumber == NULL)
                                <td>
                                    <a href="{{url('/admin/member-information/')}}/{{$value->id}}" class="btn btn-danger"><i class="fa fa-window-close"></i> ข้อมูลไม่ถูกต้อง</a>
                                </td>
                                @else
                                <td>
                                    <a href="{{url('/admin/member-information/')}}/{{$value->id}}" class="btn btn-success"><i class="fa fa-check"></i> ตรวจสอบแล้ว</a>
                                </td>
                                @endif
                                <td>
                                    <center>
                                        <form action="{{url('/admin/member/member-delete/')}}/{{$value->id}}" enctype="multipart/form-data" method="post">@csrf
